...
from relic.sga.v2.arciv import Arciv, dumps, load, ArcivEncoder, parses, parse, dump, loads
from relic.sga.v2.arciv.definitions import ArchiveHeader, TocItem, TocFolderItem, TocHeader, TocStorage, TocFolderInfo, TocFileItem
        ...
        TocItem(
            RootFolder= TocFolderItem(
                ...
                Files=[
                    TocFileItem(
                                File = "store.txt",
                                Path = Path("\Root\Samples\store.txt"),
                                Size = 0,
...
@pytest.mark.parametrize(["arciv_input","expected"], _ARCIV_LOAD_TESTS, ids=_ARCIV_LOAD_TEST_IDS)
class TestLoadArciv:
...
@pytest.mark.parametrize(["arciv_input","expected"], _ARCIV_PARSE_TESTS, ids=_ARCIV_PARSE_TEST_IDS)
...
@pytest.mark.parametrize(["expected","arciv_input"], _ARCIV_DUMP_TESTS, ids=_ARCIV_DUMP_TEST_IDS)
class TestDumpArciv:
