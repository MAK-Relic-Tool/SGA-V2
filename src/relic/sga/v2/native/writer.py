...
from relic.sga.core import StorageType, MAGIC_WORD, Version
...
class _SgaWriter(Generic[_T]):
    ...
    def write(self, sga: _T, name: str):
        self._write(sga, name)
    ...
    def _add_name(self, drive: str, name: str):
        ...
        name_pos = name_table[name] = self._name_stream.tell()
    ...
    def _write_magic_and_version(self) -> None:
        ...
        VERSION = (2, 0)
        self._out.write(VERSION_STRUCT.pack(VERSION))

    def _write_header(self, meta: ArchiveMeta) -> None:
        self._logger.debug("Writing V2 Metadata")
        buffer = ARCHIVE_HEADER.pack(
            meta.file_md5, meta.name, meta.toc_md5, meta.toc_size
        )
    ...
    def _resolve_desired_format(self, meta: ArchiveMeta):
        if self._desired_format == SgaV2GameFormat.Unknown:
    ...
    def _add_data(
        ...
        buffer = raw  # TODO; toc hasher and file hashe can be merged by stopping at the toc hash, returning a result, then continuing to hash
    ...
    def _add_toc_drive(
        ...
        buffer = DRIVE_ENTRY.pack(
            name, alias, first_folder, last_folder, first_file, last_file
        )
        self._drives += 1
    ...
    def _add_toc_folder(
        ...
        self._logger.debug("Writing Folder Entry")
    ...
    def _add_toc(self, sga: _T):
        self._logger.debug("Adding entries to TOC")
        raise NotImplementedError

    def _write(self, sga: _T, name: str):
        ...
        self._write_header(ArchiveMeta.default())
