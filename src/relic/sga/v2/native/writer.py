...
class _SgaWriter(Generic[_T]):
    ...
    TOC_START = 180
    TOC_DATA_START = (
        TOC_START + TOC_PTR.size * 4
    )  # toc data starts after the 4 pointers

    ALLOW_DUPE_NAMES = True  # standard files dont seem to check
    ...
    def __init__(
        ...
        self._name_count: int = (
            0  # we need a name count because of duplicate names in name table
        )
        self._drives = 0
    ...
    def _add_name(self, drive: str, name: str):
        ...
        if safe_name in name_table:
            if not self.ALLOW_DUPE_NAMES:
                self._logger.debug(f"Using cached name '{safe_name}'")
                return name_table[safe_name]
            else:
                self._logger.debug(f"Adding duplicate name '{safe_name}'")
    ...
    def _write_header(self, meta: ArchiveMeta, overwrite_name: str | None) -> None:
        ...
        self._out.seek(self.HEADER_START)
    ...
    def _create_ptrs(self) -> TocPointers:
        ...
        TOC_RELATIVE_DATA_START = self.TOC_DATA_START - self.TOC_START
        drives = TocPointer(
            offset=TOC_RELATIVE_DATA_START,
            count=self._drives,
    ...
    def _write_toc(self):
        ...
        self._out.seek(self.TOC_DATA_START)  # skip the pointers
        ...
        toc_size = end - self.TOC_START  # toc_size
        return toc_size

    def _write_data(self, toc_size: int):
        self._logger.debug("Writing Blob Data")
        self._out.seek(self.TOC_START + toc_size)
    ...
    def _add_metadata(self, meta: FileMetadata):
        ...
        buffer = FILE_METADATA.pack(safe_name, safe_modified, meta.crc32)
    ...
    def _add_data(
        ...
        crc_hash = crc32(raw)
        self._add_metadata(FileMetadata(name, modified, crc_hash))

        buffer = raw
        if storage in [StorageType.BUFFER_COMPRESS, StorageType.STREAM_COMPRESS]:
            buffer = zlib.compress(raw, level=9)
            # sanity = zlib.decompress(buffer)
    ...
    def _add_toc_drive(
        ...
        if alias is None:
            self._logger.warning(
                f"drive '{name}' was missing an alias; defaulting to name"
            )
            alias = name
    ...
    def _add_toc_file(
        ...
        STORAGE_WRITER = {
            SgaV2GameFormat.DawnOfWar: lambda _storage_type: int(_storage_type) << 4,
            SgaV2GameFormat.ImpossibleCreatures: lambda _storage_type: int(
                _storage_type
            ),
        }[game_format]

        packed_storage_type = STORAGE_WRITER(storage_type)
        buffer = ENTRY_FORMAT.pack(
            name_offset,
            packed_storage_type,
            data_offset,
            compressed_size,
    ...
    def _add_toc_folder(
        ...
        if folder_index is None:  # assume its a faker
            ...
            self._logger.debug(
                f"Writing Folder[{folder_index}] Entry: (name={name_offset}, first_folder={first_folder}, last_folder={last_folder}, first_file={first_file}, last_file={last_file})"
            )
        if increment_count:
