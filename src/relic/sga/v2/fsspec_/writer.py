...
class FsSpecWriter(_SgaWriter[SgaFsSpec]):
    ...
    def _add_fs_names(
        self, drive: _Directory, cur_folder: _Directory, is_root: bool = False
    ):
        if is_root:
    ...
    def _get_full_path(
        self, drive: _Directory, file_or_folder: _File | _Directory
    ) -> str:
    ...
    def _add_fs_file(
        ...
        data_offset, (decomp_size, comp_size) = self._add_data(
            name, data, storage_type, modified
        )
    ...
    def _add_fs_folder(
        ...
        if writeback_ptr is None:  # write a faker!
            ...
            folder_index = self._add_toc_folder(
                name_offset, 0, 0, 0, 0, folder_index, increment_count=True
            )
        ...
        for sub_folder in folder.sub_folders.values():
            ...
            sub_folder_index = self._add_toc_folder(
                0, 0, 0, 0, 0, increment_count=True
            )  # blank folder
        ...
        for sub_writeback, sub_folder, sub_index in sub_folders:
            self._add_fs_folder(
                drive, sub_folder, game_format, sub_writeback, sub_index
            )
    ...
    def _add_fs_drive(self, drive: _Directory, game_format: SgaV2GameFormat):
        ...
        wb_index = self._add_toc_folder(
            folder_name_offset, 0, 0, 0, 0, increment_count=True
        )
